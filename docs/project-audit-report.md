...
| 2026-08-17 | Promoted `research-recomp-boxing-v2@2.1.0` to `CURRENT_DEFAULT_PROGRAM_ID`, ending the opt-in period. Retired `legacy-v1.json` and the `weeklyPlan` constant that duplicated it; the default plan now comes from the registry via `getDefaultWorkoutPlanDays()`. Renamed the `legacy-default` program source to `registry-default` and the `Legacy` catalog badge to `Default`. Installed programs and custom plans are unaffected. | 5, 6, 17, 19, Workout Program Upgrade Progress | Both verification scripts updated and passing (457 assertions; reset contamination now probed with V2.1 content); lint, typecheck, and build passed; headless local-mode run confirmed V2.1 on Today's Workout, Weekly Plan, and Plan Editor with a two-program catalog and 0 console errors |
| 2026-08-17 | Removed 40 unreferenced declarations across `src/`, found by scanning every identifier in `src/` and `scripts/`: 32 exported functions with no callers, 8 types and private helpers orphaned by those removals, and the unreferenced `PlaceholderPage` and `OfflineFallback` components. `clearNamespacedData` ("forget this device") and `restoreLocalStorageBackup` were placeholders for features never wired to the UI and are recoverable from history. No behavior changed. | 19 | Lint, typecheck, build, and both verification scripts passed; a re-scan reports no fully-unused exports remaining |
